...
// 	require_once ('../src/cPeriod.class.php' );
	...
	require_once ('../src/cDateStrategyNoInterval.class.php' );
     } catch( Exception $e ) {
...
class cDateStrategyNoIntervalTest extends PHPUnit_Framework_TestCase {
	...
	// create new object
	$strategy = new \rstoetter\libdatephp\cDateStrategyNoInterval( );
	...
	assert( $strategy->AddDate( new \rstoetter\libdatephp\cDate( 4, 8, 2016 ) ) );
	assert( $strategy->AddDate( new \rstoetter\libdatephp\cDate( 7, 8, 2016 ) ) );
	assert( $strategy->AddDate( new \rstoetter\libdatephp\cDate( 5, 8, 2016 ) ) );

	assert( $strategy->IsDate( new \rstoetter\libdatephp\cDate( 5, 8, 2016 ) ) );
	assert( ! $strategy->IsDate( new \rstoetter\libdatephp\cDate( 5, 11, 2016 ) ) );
	...
	$dt_first = new \rstoetter\libdatephp\cDate( 4, 8, 2016 );
	...
	$dt_first = new \rstoetter\libdatephp\cDate( 7, 8, 2016 );

	$strategy->GetFollower( $dt_first, $dt_next, \rstoetter\libdatephp\cDateStrategy::DIRECTION_BACKWARD );
	echo "\n " . $dt_first->AsSQL( );
	if ( $dt_first->AsSQL( ) != '2016-05-08') die( "\n Test nicht bestanden" );
  ...
  public function testcDateStrategyNoInterval( )
  {
     try {
...
} 	// class cDateStrategyNoIntervalTest
